...
X_adaline = (X - np.min(X)) / (np.max(X) - np.min(X))   # min-max para ADALINE e Perceptron
X_mlp = (X - np.mean(X)) / (np.std(X) + 1e-12)          # z-score para MLP
...
for i in range(rodadas):
    ...
    X_train_adaline, X_test_adaline = X_adaline[:, train_idx], X_adaline[:, test_idx]
    ...
    W_perceptron = codigo.simplePerceptron(X_train_adaline, y_train_s, epocas_max=epocas_max_Simple, lr=learningR_Simple)
    X_test_perceptron = np.concatenate((-np.ones((1, X_test_adaline.shape[1])), X_test_adaline), axis=0)
    ...
    W_adaline = codigo.ADALINE(X_train_adaline, y_train_s, epocas_max=epocas_max_ADALINE, lr=learningR_ADALINE, pr=precisionR_ADALINE)
    u_adaline = W_adaline.T @ np.concatenate((-np.ones((1, X_test_adaline.shape[1])), X_test_adaline), axis=0)
    ...
    model_mlp_tanh, precisions_mlp = codigo.MLP(X_train_mlp, y_train_oh,
                                                hidden_layers=hidden_layers,
                                                learning_rate=learningR_MLP,
                                                epocas_max=epocas_max_MLP,
                                                activation=activation_func)
    Y_pred_mlp_tanh = codigo.MLP_predict(model_mlp_tanh, X_test_mlp)
