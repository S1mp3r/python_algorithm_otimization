...
# acuracias = {"Perceptron": [], "ADALINE": [], "MLP - SIGMOID": [], "MLP - TANH": []}
# matrizes = {"Perceptron": [], "ADALINE": [], "MLP - SIGMOID": [], "MLP - TANH": []}

acuracias = {"Perceptron": [], "ADALINE": [], "MLP - TANH": []}
matrizes = {"Perceptron": [], "ADALINE": [], "MLP - TANH": []}

acuracias2 = {"Perceptron": [], "ADALINE": [], "MLP - TANH": []}
matrizes2 = {"Perceptron": [], "ADALINE": [], "MLP - TANH": []}
...
for rodada in range(max_rounds):
    ...
    # MLP - Sigmoid
    # model_mlp_sigmoid, losses_mlp_sigmoid = codigo.MLP(X_train,
    #                        y_train,
    ...
    # y_pred_mlp_sigmoid = codigo.MLP_predict(model_mlp_sigmoid, X_test)
    # y_pred_mlp_bin_sigmoid = np.array([codigo.sign_ajustavel(u, first=1, second=0.5, third=0) for u in y_pred_mlp_sigmoid.flatten()])

    # acc_mlp_sigmoid, esp_mlp_sigmoid, sens_mlp_sigmoid, matriz_mlp_sigmoid = calcular_metricas(y_test_bin, y_pred_mlp_bin_sigmoid)
    # acuracias["MLP - SIGMOID"].append(acc_mlp_sigmoid)
    # matrizes["MLP - SIGMOID"].append(matriz_mlp_sigmoid)
    ...
    w_perceptron = codigo.simplePerceptron_G(X_train,
                                           y_train,
                                           epocas_max=epocas_max,
                                           lr=learningR
                                           )[-1]
    ...
    acc_p, esp_p, sens_p, matriz_p = calcular_metricas(y_test_bin, y_pred_perceptron_bin)
    acuracias2["Perceptron"].append(acc_p)
    matrizes2["Perceptron"].append(matriz_p)

    # ADALINE
    w_adaline = codigo.ADALINE_G(X_train,
                               y_train,
                               epocas_max=epocas_max,
                               lr=learningR,
                               pr=precisionR
    ...
    acc_a, esp_a, sens_a, matriz_a = calcular_metricas(y_test_bin, y_pred_adaline_bin)
    acuracias2["ADALINE"].append(acc_a)
    matrizes2["ADALINE"].append(matriz_a)
    ...
    # MLP - Tanh
    model_mlp_tanh, precision_mlp_tanh = codigo.MLP_G(X_train,
                           y_train,
                           hidden_layers=hidden_layers,
                           learning_rate=learningR,
                           epocas_max=epocas_max_MLP,
    ...
    y_pred_mlp_tanh = codigo.MLP_predict_G(model_mlp_tanh, X_test)
    y_pred_mlp_bin_tanh = np.array([codigo.sign_ajustavel(u, first=1, second=0, third=-1) for u in y_pred_mlp_tanh.flatten()])

    acc_mlp_tanh, esp_mlp_tanh, sens_mlp_tanh, matriz_mlp_tanh = calcular_metricas_negativas(y_test_bin_negativo, y_pred_mlp_bin_tanh)
    acuracias2["MLP - TANH"].append(acc_mlp_tanh)
    matrizes2["MLP - TANH"].append(matriz_mlp_tanh)
...
for modelo in acuracias:
    ...
    matriz_media = np.mean(matrizes2[modelo], axis=0)
    matriz_media_int = matriz_media.astype(int)
    acuracia_media = np.mean(acuracias2[modelo])
    especificidade_media = matriz_media[0, 0] / (matriz_media[0, 0] + matriz_media[0, 1])
    sensibilidade_media = matriz_media[1, 1] / (matriz_media[1, 1] + matriz_media[1, 0])
...
# plt.plot(range(1, len(losses_mlp_sigmoid) + 1), losses_mlp_sigmoid, label="Loss (Erro Quadrático Médio)", color='blue')
# plt.title("Curva de Aprendizado - MLP - SIGMOID")
# plt.xlabel("Épocas")
...
# plt.figure(figsize=(10, 6))
# plt.plot(range(1, len(precision_mlp_tanh) + 1), precision_mlp_tanh, label="Precision (Erro Quadrático Médio)", color='blue')
# plt.title("Curva de Aprendizado - MLP - TANH")
# plt.xlabel("Épocas")
# plt.ylabel("Erro Quadrático Médio (Loss)")
# plt.grid(True)
# plt.legend()
